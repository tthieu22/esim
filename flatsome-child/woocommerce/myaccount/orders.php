...
<?php if ( $has_orders ) : ?>
	...
	<table class="woocommerce-orders-table woocommerce-MyAccount-orders shop_table shop_table_responsive my_account_orders account-orders-table my_account_orders-custom">
		<thead>
		...
		<tbody class="order-table-cs ">
			<?php
			foreach ( $customer_orders->orders as $customer_order ) {
				$order      = wc_get_order( $customer_order ); // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
				$item_count = $order->get_item_count() - $order->get_item_count_refunded();
				?>
				<tr class="order-table-cs woocommerce-orders-table__row woocommerce-orders-table__row--status-<?php echo esc_attr( $order->get_status() ); ?> order">
					<?php foreach ( wc_get_account_orders_columns() as $column_id => $column_name ) : ?>
						<td class="woocommerce-orders-table__cell woocommerce-orders-table__cell-<?php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); ?>">
							...
							<?php elseif ( 'order-total' === $column_id ) : ?>
								...
								echo wp_kses_post( sprintf( _n( '', '', $item_count, 'woocommerce' ), $order->get_formatted_order_total(), $item_count ) );
								?>
								...
								<?php elseif ('order-actions' === $column_id) : ?>
									...
									if (!empty($actions)) {
										foreach ($actions as $key => $action) {
											// Define Font Awesome icons for each action.
											$icon_classes = array(
												'view' => 'fas fa-angle-down',
												'cancel' => 'fas fa-times',
												// Add more icons for other actions as needed.
											);

											// Check if the action key has a corresponding Font Awesome icon class.
											if (isset($icon_classes[$key])) {
												echo '<a href="' . esc_url($action['url']) . '" class="view-detail ' . sanitize_html_class($key) . '">';
												echo '<i class="' . esc_attr($icon_classes[$key]) . '"></i>';
												echo '</a>';
											} else {
												// If no Font Awesome icon is defined for the action, display the action name as text.
